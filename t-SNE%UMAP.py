...
#Step 1: Generate synthetic data with four clusters in a 3D space
# CLuster centers:
# Basically we are making 4 centers' coordinates for the four clusters.
centers = [ [ 2, -6, -6],
            [-1,  9,  4],
            [-8,  7,  2],
            [ 4,  7,  9] ]

# Cluster standard deviations:(Lower values means more tight, compact; Higher values means dispersed and spread out)
cluster_std=[1,1,2,3.5]

# Make the blobs and return the data and the blob labels. 
# X represent the 500 points' coordinates, while labels shows what cluster each point belongs to.
X, labels_ = make_blobs(n_samples=500, centers=centers, n_features=3, cluster_std=cluster_std, random_state=42)

# Create a DataFrame for Plotly, for all of the 500 points.
...
#Import the TSNE model
tsne = TSNE(n_components=2, random_state=42, perplexity=30, max_iter=1000)
X_tsne = tsne.fit_transform(X_scaled)

# Create a 2D graph of the result of TSNE model, which turns those 3D into 2D.
